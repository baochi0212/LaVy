    ...
    def __getitem__(self, index):
        with jsonlines.open(self.jsonl_path) as reader:
            for i, line in enumerate(reader):
                if i == index:
                    image_path = "./data/dev-images/" + line['image_id']
                    ...
                    return input_ids, image_tensor, image.size, self.tokenizer(question, return_tensors='pt').input_ids, self.tokenizer(answer, return_tensors='pt').input_ids


def collate_fn(batch):
    input_ids, image_tensors, image_sizes, question, answer = zip(*batch)
    input_ids = torch.stack(input_ids, dim=0)
    image_tensors = torch.stack(image_tensors, dim=0)
    question = torch.stack(question, dim=0)
    answer = torch.stack(answer, dim=0)
    return input_ids, image_tensors, image_sizes, question, answer
...
def eval_model(args):
    ...
    for input_ids, image_tensor, image_sizes, question, answer in tqdm(data_loader):
        ...
        writer.write({"question": question, "label": answer, "pred": outputs})
        # ans_file.flush()
    #ans_file.close()
